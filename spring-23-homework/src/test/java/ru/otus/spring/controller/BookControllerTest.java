...
class BookControllerTest {
    ...
    @Test
    void shouldRedirectOnLogin() throws Exception {
        mvc.perform(get("/booklist"))
                .andExpect(status().is3xxRedirection())
        ...
        mvc.perform(post("/bookedit")
                        .queryParam("id", expectedBooks.get(0).getId())
                        .param("name", expectedBooks.get(0).getName()))
                .andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrl("http://localhost/login"));
        ...
        mvc.perform(post("/bookadd")
                        .contentType(MediaType.APPLICATION_FORM_URLENCODED)
                        .param("name", expectedBooks.get(0).getName())
                        .param("authorsId", expectedBooks.get(0).getAuthors().get(0).getId())
                        .param("genresId", expectedBooks.get(0).getGenres().get(0).getId())
                )
                .andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrl("http://localhost/login"));
